...
for suffix in params:
    ...
    bestC, f_num_layers, f_hidden_size, f_batch_size, f_lr, f_wd = hyps[suffixcount]
    ...
    for splitidx in range(idxmin, idxmax):
        ...
        train_dataset = CurrDataset("train", {'features':torch.tensor(X_train_new), 'labels':torch.tensor(y_train_new), 'uncensored_indicator':torch.tensor(binary_y_train_new), 'propensity':torch.tensor(propensity_train)})
        train_loader = data_utils.DataLoader(train_dataset,
                                             batch_size=f_batch_size,
                                             shuffle=True)


        val_dataset = CurrDataset("train", {'features':torch.tensor(X_val_new), 'labels':torch.tensor(y_val_new), 'uncensored_indicator':torch.tensor(binary_y_val_new), 'propensity':torch.tensor(propensity_val)})
        ...
        for epoch in range(500):
            ...
            torch.save(model, '/data4/meerak/support50_propbin_models/icind_epoch%d_idxmin%d'%(epoch, idxmin))
        ...
        model = DeepHit(X_train_new.shape[1], max(y_train_new), f_num_layers, f_hidden_size).to(device)
        model = torch.load('/data4/meerak/support50_propbin_models/icind_epoch%d_idxmin%d'%(best_epoch, idxmin))
        ...
        joblib.dump(test_preds, '/data4/meerak/support50_propbin_test_preds/icind_y_test_pred_split%d_%s.joblib'%(splitidx, suffix))
        ...
        os.system(f"rm -f {folder_path}/*icind*idxmin{idxmin}*")