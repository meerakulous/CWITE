...
for suffix in params:
    ...
    for splitidx in range(idxmin, idxmax):
        set_seed(splitidx)
        X_train_new, y_train_new, X_val_new, y_val_new = random_train_val_split(
    X_combined, y_combined, n_train, n_val)

        
        train_dataset = CurrDataset("train", {'features':torch.tensor(X_train_new), 'labels':torch.tensor(y_train_new)})
        train_loader = data_utils.DataLoader(train_dataset,
                                             batch_size=f_batch_size,
                                             shuffle=True)


        val_dataset = CurrDataset("train", {'features':torch.tensor(X_val_new), 'labels':torch.tensor(y_val_new)})
