...
for suffix in params:
    ...
    for splitidx in range(idxmin, idxmax):
        ...
        for epoch in range(500):
            ...
            torch.save(prop, output_path('support50_propbin_models', 'proposed_largeweight_linear_epoch%d_idxmin%d')%(epoch, idxmin))
        ...
        prop = IPCW(X_train_new.shape[1], max(y_train_new), f_num_layers, f_hidden_size).to(device)
        prop = torch.load(output_path('support50_propbin_models', 'proposed_largeweight_linear_epoch%d_idxmin%d')%(best_epoch, idxmin))
        ...
        joblib.dump(test_preds, output_path('support50_propbin_test_preds', 'proposed_largeweight_linear_y_test_pred_split%d_%s.joblib')%(splitidx, suffix))
