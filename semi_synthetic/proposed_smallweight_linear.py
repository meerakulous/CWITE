...
for suffix in params:
    ...
    bestC, elbow_k, f_num_layers, f_hidden_size, f_batch_size, f_lr, f_wd, f_lambda_loss, f_func = hyps[suffixcount]
    ...
    for splitidx in range(idxmin, idxmax):
        ...
        for epoch in range(500):
            ...
            torch.save(prop, '/data4/meerak/support50_propbin_models/proposed_smallweight_linear_epoch%d_idxmin%d'%(epoch, idxmin))
        ...
        prop = IPCW(X_train_new.shape[1], max(y_train_new), f_num_layers, f_hidden_size).to(device)
        prop = torch.load('/data4/meerak/support50_propbin_models/proposed_smallweight_linear_epoch%d_idxmin%d'%(best_epoch, idxmin))
        ...
        joblib.dump(test_preds, '/data4/meerak/support50_propbin_test_preds/proposed_smallweight_linear_y_test_pred_split%d_%s.joblib'%(splitidx, suffix))
        ...
        os.system(f"rm -f {folder_path}/*proposed_smallweight_linear*idxmin{idxmin}*")
