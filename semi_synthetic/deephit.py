...
X_val_new, y_val_new, orig_y_val_new, binary_y_val_new = random_train_val_split(
    X_combined, y_combined, orig_y_combined, binary_y_combined, n_train, n_val)
        ...
        for epoch in range(500):
            ...
            torch.save(model, '/data4/meerak/support50_propbin_models/dhcind_epoch%d_idxmin%d'%(epoch, idxmin))
        ...
        model = DeepHit(X_train_new.shape[1], max(y_train_new), f_num_layers, f_hidden_size).to(device)
        model = torch.load('/data4/meerak/support50_propbin_models/dhcind_epoch%d_idxmin%d'%(best_epoch, idxmin))
        ...
        joblib.dump(test_preds, '/data4/meerak/support50_propbin_test_preds/dhcind_y_test_pred_split%d_%s.joblib'%(splitidx, suffix))
