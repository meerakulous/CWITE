...
def create_cluster_weights(binary_y, y, categories, func):
    ...
    for i in range(len(y)):
        ...
        if binary_y[i] == 0:
            updated_y.append(max_cluster_y)
        else:
            updated_y.append(y[i])
    
    return np.array(updated_y), np.array(curr_cluster_weights)
...
for COUNT in range(idx_min, idx_max):
    ...
    for grididx in range(10):
        ...
        y_train, train_cluster_weight = create_cluster_weights(binary_y_train, y_train, categories_train, func)
        y_val, val_cluster_weight = create_cluster_weights(binary_y_val, y_val, categories_val, func)
        y_test, test_cluster_weight = create_cluster_weights(binary_y_test, y_test, categories_test, func)
    ...
    for epoch in range(500):
        ...
        for batch_idx, (data, label, uci, propensity, cluster_weight) in enumerate(val_loader):
            ...
            curr_val_losses.append(loss.detach().cpu().numpy())
        val_losses.append(np.mean(curr_val_losses))
        torch.save(prop, '/data4/meerak/onevar_models/proposed_assignMAX_epoch%d'%(epoch))
    ...
    prop = IPCW(X_train.shape[1], max(y_train), f_num_layers, f_hidden_size).to(device)
    prop = torch.load('/data4/meerak/onevar_models/proposed_assignMAX_epoch%d'%(best_epoch))
    ...
    joblib.dump(test_preds, '/data4/meerak/onevar_test_preds/proposed_assignMAX_y_test_pred_%s.joblib'%(suffix))
