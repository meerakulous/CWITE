...
for COUNT in range(idx_min, idx_max):
    ...
    for epoch in range(500):
        ...
        for batch_idx, (data, label) in enumerate(val_loader):
            ...
            curr_val_losses.append(loss.detach().cpu().numpy())
        val_losses.append(np.mean(curr_val_losses))
        torch.save(model, '/data4/meerak/onevar_models/oracle_epoch%d'%epoch)
    ...
    model = DeepHit(X_train.shape[1], max(y_train), f_num_layers, f_hidden_size).to(device)
    model = torch.load('/data4/meerak/onevar_models/oracle_epoch%d'%(best_epoch))
    ...
    joblib.dump(test_preds, '/data4/meerak/onevar_test_preds/oracle_y_test_pred_%s.joblib'%(suffix))
