...
f = open('pmf_extract_proposed_orig_hyp_%d_%d.txt'%(idx_min, idx_max), 'w')    
...
for COUNT in range(idx_min, idx_max):
    ...
    for epoch in range(500):
        ...
        for batch_idx, (data, label, uci, propensity, cluster_weight) in enumerate(val_loader):
            ...
            curr_val_losses.append(loss.detach().cpu().numpy())
        val_losses.append(np.mean(curr_val_losses))
        torch.save(prop, '/data4/meerak/onevar_models_pmf_extract/proposed_epoch%d'%(epoch))
    ...
    prop = IPCW(X_train.shape[1], max(y_train), f_num_layers, f_hidden_size).to(device)
    prop = torch.load('/data4/meerak/onevar_models_pmf_extract/proposed_epoch%d'%(best_epoch), weights_only=False)
    ...
    for batch_idx, (data, label, uci, propensity, cluster_weight) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            pmf = F.softmax(prop.feature_extractor(data), dim=1)
    ...
    joblib.dump(test_preds, '/data4/meerak/onevar_test_preds_pmf_extract/proposed_y_test_pred_%s.joblib'%(suffix))
    joblib.dump(test_pmfs, '/data4/meerak/onevar_test_pmfs/proposed_y_test_pmf_%s.joblib'%(suffix))
