...
for COUNT in range(idx_min, idx_max):
    ...
    for epoch in range(500):
        ...
        for batch_idx, (data, label) in enumerate(val_loader):
            ...
            curr_val_losses.append(loss.detach().cpu().numpy())
        val_losses.append(np.mean(curr_val_losses))
        torch.save(model, output_path('onevar_models_pmf_extract', 'oracle_epoch%d')%epoch)
    ...
    model = DeepHit(X_train.shape[1], max(y_train), f_num_layers, f_hidden_size).to(device)
    model = torch.load(output_path('onevar_models_pmf_extract', 'oracle_epoch%d')%(best_epoch), weights_only=False)
    ...
    joblib.dump(test_preds, output_path('onevar_test_preds_pmf_extract', 'oracle_y_test_pred_%s.joblib')%(suffix))
    joblib.dump(test_pmfs, output_path('onevar_test_pmfs', 'oracle_y_test_pmf_%s.joblib')%(suffix))
